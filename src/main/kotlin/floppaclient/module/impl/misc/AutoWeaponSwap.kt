...
object AutoWeaponSwap : Module(
    ...
    private val fromInv = BooleanSetting("From Inv", false, description = "Lets you use the weapons from your inventory.")
    ...
    private val reforges = setOf("suspicious", "fabled", "heroic", "spicy", "withered")
  /*  private val leftClickItems = setOf("Suspicious", "Fabled", "Heroic", "Spicy", "Withered")
    ...
    fun onLeftClick(event: ClickEvent.LeftClickEvent) {
        ...
        if (heldItem?.displayName?.containsOneOf(leftClickBlacklist) == true) return
        if (heldItem?.reforge?.containsOneOf(reforges) == true ) {
